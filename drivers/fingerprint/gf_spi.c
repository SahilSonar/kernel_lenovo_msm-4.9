...
static void notification_work(struct work_struct *work)
{
	...
	gf_dev=container_of(work, struct gf_dev, work);

	
	input_report_key(gf_dev->input, GF_KEY_INERRUPT, 1);
	input_sync(gf_dev->input);
	input_report_key(gf_dev->input, GF_KEY_INERRUPT, 0);
	input_sync(gf_dev->input);
