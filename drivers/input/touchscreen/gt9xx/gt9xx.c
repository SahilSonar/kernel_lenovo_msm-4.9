...
#define GESTURE_NUM  (10)
#endif

#ifdef  QIKU_GEASTURE

#define TW_SUPPORT_DOUBLE_CLICK_WAKEUP   0x01
#define TW_SUPPORT_UP_SLIDE_WAKEUP 		0x02
#define TW_SUPPORT_DOWN_SLIDE_WAKEUP 	0X04
#define TW_SUPPORT_C_SLIDE_WAKEUP 		0x08
#define TW_SUPPORT_E_SLIDE_WAKEUP 		0x10
#define TW_SUPPORT_M_SLIDE_WAKEUP 		0x20
#define TW_SUPPORT_O_SLIDE_WAKEUP 		0x40
#define TW_SUPPORT_W_SLIDE_WAKEUP 		0x80
#define TW_SUPPORT_RIGHT_SLIDE_WAKEUP     0x100
#define TW_SUPPORT_LEFT_SLIDE_WAKEUP         0x200

int gtp_support_gesture;//
...
struct slide_wakeup gtp_ges_wakeup[]={
...
static ssize_t gesture_ctrl_store(struct device *dev, struct device_attribute *attr,
...
static ssize_t gesture_enabled_store(struct device *dev, struct device_attribute *attr,
			    const char *buf, size_t size)
{   
	char str[15];
    sscanf(buf, "%s",str);
	
	if (!strncmp("enable", str, strlen(str)))
		gesture_enabled=1;
	else 
		gesture_enabled=0;
...
static ssize_t gesture_enabled_show(struct device *dev, struct device_attribute *attr,
			   char *buf)
{  
	char str[15];
    ...
    return snprintf(buf,strlen(str)+2,"%s\n",str);
	
}

static ssize_t gesture_wakeup_show(struct device *dev, struct device_attribute *attr,
			   char *buf)
{
    char str_buf[32],ges_buf[60];
	int i;
	
	memset(ges_buf, 0, 60);
	for(i=0;i<GESTURE_NUM;i++)
    {
    	if(gtp_support_gesture & (1<<i))
		{
		  	memset(str_buf, 0, 32);
		  	
    		snprintf(str_buf,strlen(gtp_ges_wakeup[i].gesture)+2,"%s,",gtp_ges_wakeup[i].gesture);
		   
			strncat(ges_buf,str_buf,strlen(str_buf));
	...
	return snprintf(buf,strlen(ges_buf)+2,"%s\n",ges_buf);
}

static DEVICE_ATTR_WO(gesture_ctrl);
static DEVICE_ATTR_RO(gesture_wakeup);
static DEVICE_ATTR_RW(gesture_enabled);

static struct attribute *touchscreen_attrs[] = {
	&dev_attr_gesture_ctrl.attr,
	&dev_attr_gesture_wakeup.attr,
	&dev_attr_gesture_enabled.attr,
	NULL,
};
ATTRIBUTE_GROUPS(touchscreen);
...
u8 config[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH]
    ...
    //static const u16 key_xy_array[3][2] = {{180,2040},{540,2040},{900,2040}};//menu,homepage,back
  ...
  //  char gtp_gesture_value;
    ...
    {               
        ...
        {   
				...
				for(i=0;i<GESTURE_NUM;i++)
				   { 
				   		if ((doze_buf[2] == gtp_ges_wakeup[i].gesture_code)&& (gtp_support_gesture & (1<<i)))
					    {
							envp = gtp_ges_wakeup[i].envp;
  							kobject_uevent_env(&ts->touch_dev->kobj, KOBJ_CHANGE, envp);
			...
			if(key_value & (0x01<<i)) 
	     	{
				...
				gtp_touch_down(ts,0,key_xy_array[i][0],key_xy_array[i][1],0);
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
	...
	ret = GTP_GPIO_REQUEST(gtp_vcc_gpio, "GTP VCC PORT");
    if (ret < 0) 
    {
        GTP_ERROR("Failed to request VCC GPIO:%d, ERRNO:%d",(s32)gtp_vcc_gpio,ret);
        ret = -ENODEV;
    }
...
static void gtp_parse_dt(struct device *dev)
{
	...
	gtp_vcc_gpio = of_get_named_gpio(np, "goodix,vcc-gpio", 0);
...
static int gtp_power_switch(struct i2c_client *client, int on)
{
	...
	if (on) {
		...
		GTP_GPIO_OUTPUT(gtp_vcc_gpio, 1);
		udelay(2);
		...
		GTP_GPIO_OUTPUT(gtp_vcc_gpio, 0);
	}
	return ret;
...
//	
    ...
    gt9xx_gesture_init_node();
		...
		ts->touch_class = class_create(THIS_MODULE, "touchscreen");
		ts->touch_class->dev_groups = touchscreen_groups;
		ts->touch_dev = device_create(ts->touch_class, NULL, MKDEV(255, 1), NULL, "touchscreen_dev");
		if (IS_ERR(ts->touch_dev )) {
        		printk("create device for fpsensor failed\n");
			ret = PTR_ERR(ts->touch_dev );
			class_destroy(ts->touch_class);
			ts->touch_class=NULL;
...
if(err == 0){
    ...
    if (ret < 0)
    {
        GTP_ERROR("GTP works in polling mode.");
    }
    else
    {
...
//#endif
	...
	//sprintf(tp_info,"EACH,GT915L_%x,V%d",  version_info,cfg_version);
...
//free fpio
	...
	if (gpio_is_valid(gtp_vcc_gpio))
		gpio_free(gtp_vcc_gpio);
