...
extern u8 fts_gesture_data_mode_flag;
...
static int fts_power_source_init(struct fts_ts_data *data)
{
	...
	/* request vcc gpio */
	if (gpio_is_valid(data->pdata->vcc_gpio)) {
		ret = gpio_request(data->pdata->vcc_gpio, "fts_vcc_gpio");
		if (ret) {
			FTS_ERROR("[GPIO]vcc gpio request failed");
			goto err_set_vtg_vdd;
		}
	
		ret = gpio_direction_output(data->pdata->vcc_gpio, 0);
		if (ret) {
			FTS_ERROR("[GPIO]set_direction for vcc gpio failed");
			goto err_vcc_gpio_dir;
		}
	}
...
static int fts_power_source_ctrl(struct fts_ts_data *data, int enable)
{
    ...
    if (enable) {
        if (data->power_disabled) {
			...
			ret = gpio_direction_output(data->pdata->vcc_gpio, 1);
			if (ret) {
				FTS_ERROR("[GPIO]set_vcc_gpio_direction 1 failed");
			}
        ...
        if (!data->power_disabled) {
			...
			ret = gpio_direction_output(data->pdata->vcc_gpio, 0);
			if (ret) {
				FTS_ERROR("[GPIO]set_vcc_gpio_direction 0 failed");
			}
...
static int fts_input_report_b(struct fts_ts_data *data)
{
    ...
    for (i = 0; i < data->touch_point; i++) {
        ...
        if (EVENT_DOWN(events[i].flag)) {
            ...
            FTS_DEBUG_LOG("[B]P%d(%d, %d)[p:%d,tm:%d] DOWN!", events[i].id, events[i].x,
                      events[i].y, events[i].p, events[i].area);
        } else {
            uppoint++;
            input_mt_report_slot_state(data->input_dev, MT_TOOL_FINGER, false);
            data->touchs &= ~BIT(events[i].id);
            FTS_DEBUG_LOG("[B]P%d UP!", events[i].id);
        }
    }
...
#if FTS_GESTURE_EN
    if(fts_gesture_data_mode_flag == ENABLE)
    {
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    pdata->vcc_gpio = of_get_named_gpio_flags(np, "focaltech,vcc-gpio", 0, &pdata->vcc_gpio_flags);
    if (pdata->vcc_gpio < 0)
        FTS_ERROR("Unable to get vcc_gpio");
...
#define FTS_INI_FILE_NAME		     "Focal_86255.ini"
...
#endif
#if CTP_PROC_INTERFACE
static ssize_t ctp_open_proc_read(struct file *file, char __user *user_buf,size_t count, loff_t *ppos)
{
    //char buf[15] = {'\0'};
    char *ptr = user_buf;
    u8 result= 0;
	...
	*ppos += count;
	...
	nret = fts_open_short_test( FTS_INI_FILE_NAME );
	if(0 != nret)
	{
		result = 0;
	...
	//nret = snprintf(buf, sizeof(buf)-1,"result=%d\n",result);
    nret=sprintf(ptr, "result=%d\n",result);
	if (copy_to_user(user_buf, ptr, nret+1)) {
...
static void create_ctp_proc(void)
{
    ...
    {
        FTS_ERROR("ft5x06: create parent_proc fail");
        return;
    }
    ...
    {
        FTS_ERROR("ft5x06: create open_proc fail\n");
    }
...
#define FTS_PROC_SMWP_FILE "gesture_onoff"
#define FTS_PARENT_PROC_NAME  "touchscreen_feature"
...
static ssize_t fts_gesture_proc_read(struct file *file, char __user *buf,size_t count, loff_t *ppos);
static ssize_t fts_gesture_proc_write(struct file *filp, const char __user *user_buf,size_t count, loff_t *ppos);

static  struct file_operations fts_gesture_procs_fops =
{
...
static ssize_t fts_gesture_proc_read(struct file *file, char __user *user_buf,size_t count, loff_t *ppos)
{
    char *ptr = user_buf;
    int nret = -1;
    struct input_dev *input_dev = fts_data->input_dev;
    ...
    *ppos += count;
    ...
    nret = snprintf(ptr, PAGE_SIZE, "Gesture Mode: %s\n", fts_gesture_data.mode ? "On" : "Off");
...
static ssize_t fts_gesture_proc_write(struct file *filp,  const char __user *user_buf,size_t count, loff_t *ppos)
{
    const char *ptr = user_buf;
    int nret = -1;
    char buf[100] = {0};
    struct input_dev *input_dev = fts_data->input_dev;
    printk("[GESTURE]start fts gesture\n");
    if(*ppos) {
        FTS_ERROR("tp gesture return\n");
        return 0;
    }
    *ppos += count;
    ...
    if (count >= 100) {
        FTS_ERROR("no command exceeds 80 chars.");
        return -EFAULT;
    }
    ...
    mutex_lock(&input_dev->mutex);
    if (buf[0] == '1') {   
        printk("[GESTURE]enable fts gesture\n");
        fts_gesture_data.mode = ENABLE;
        fts_gesture_data_mode_flag = fts_gesture_data.mode;
    } else if (buf[0] == '0') { 
        printk("[GESTURE]disable fts gesture\n");
        fts_gesture_data.mode = DISABLE;
        fts_gesture_data_mode_flag = fts_gesture_data.mode;
    }
   mutex_unlock(&input_dev->mutex);
    kfree(ptr);
    printk("[GESTURE]end fts gesture\n");
    return nret;
}

static void create_gesture_proc(void)
{
    struct proc_dir_entry *fts_gesture_device_proc = NULL;
    struct proc_dir_entry *fts_gesture_proc = NULL;
   
    fts_gesture_device_proc = proc_mkdir(FTS_PARENT_PROC_NAME, NULL);
    if(fts_gesture_device_proc == NULL)
    {
        FTS_ERROR("ft5x06: create gesture_parent_proc fail");
        return;
    }

    fts_gesture_proc = proc_create(FTS_PROC_SMWP_FILE, 0666, fts_gesture_device_proc, &fts_gesture_procs_fops);
    if (fts_gesture_proc == NULL)
    {
        FTS_ERROR("ft5x06: create gesture_open_proc fail\n");
    }
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    snprintf(firmware_ver,HARDWARE_MAX_ITEM_LONGTH,"ShenYue,FT5536,FW:0x%x",fw_ver_in_tp);
    ret = hardwareinfo_set_prop(HARDWARE_TP,firmware_ver);
    if (ret < 0)
    {
        FTS_ERROR("[TP] Failed to hardwareinfo_set_prop, %d\n",ret);
    }
#endif
...
static int fts_ts_suspend(struct device *dev)
{
    ...
    ts_data->suspended = true;
    //FTS_FUNC_EXIT();
    FTS_ERROR("suspend OK!!");
	FTS_DEBUG_LOG("suspend OK!!");
    return 0;
}
