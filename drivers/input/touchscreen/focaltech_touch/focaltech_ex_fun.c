...
static ssize_t fts_debug_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    sscanf(buf, "%s", str);
	if(0 == strncmp("open", str, strlen(str))){
		debug_open = true;
	}
    if(0 == strncmp("close", str, strlen(str))){
		debug_open = false;
	}
...
static DEVICE_ATTR(fts_debug, S_IRUGO | S_IWUSR, fts_debug_show, fts_debug_store);
