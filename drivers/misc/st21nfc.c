...
static int st21nfc_dev_open(struct inode *inode, struct file *filp)
{
	...
	printk("%s : device_open = %d", __func__, device_open);
	printk("%s : %d,%d ", __func__, imajor(inode), iminor(inode));
...
static int nfc_parse_dt(struct device *dev, struct st21nfc_platform_data *pdata)
{
	...
	printk("[dsc]%s : get reset_gpio[%d], irq_gpio[%d],polarity_mode[%d]\n",
...
static int __init st21nfc_dev_init(void)
{
	...
	if(strstr(saved_command_line,"board_id=P86255CA1")) {
                printk("boardid is P86255CA1");
		return i2c_add_driver(&st21nfc_driver);
        }
	else{
		printk("boardid is NOT P86255CA1, not add nfc driver");
		return -1;
	}
}
...
{
	printk("Unloading st21nfc driver\n");
	if(strstr(saved_command_line,"board_id=P86255CA1")) {
