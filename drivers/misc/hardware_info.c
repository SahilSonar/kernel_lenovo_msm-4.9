...
static long hardwareinfo_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
	...
	case HARDWARE_BT_GET:
		hardwareinfo_set_prop(HARDWARE_BT, "WCN3680"); 
		hardwareinfo_num = HARDWARE_BT;
		break;
	case HARDWARE_WIFI_GET:
		hardwareinfo_set_prop(HARDWARE_WIFI, "WCN3680"); 
	...
	case HARDWARE_GPS_GET:
		hardwareinfo_set_prop(HARDWARE_GPS, "WCN3680"); 
	    hardwareinfo_num = HARDWARE_GPS;
		break;
	case HARDWARE_FM_GET:
		hardwareinfo_set_prop(HARDWARE_FM, "WCN3680"); 
	...
	if (strlen(hardwareinfo_name[hardwareinfo_num]) > HARDWARE_MAX_ITEM_LONGTH ) {
		//pr_err("err,len:%d",strlen(hardwareinfo_name[hardwareinfo_num]));
		ret = -EINVAL;
		goto err_out;
	}
